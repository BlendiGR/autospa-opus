...
import { useTyreCounts, useCustomerCount } from '@/hooks';
import DashboardCard from './dashboardCard';
import { useTranslations } from 'next-intl';

export default function DashboardStats() {
    const t = useTranslations('Dashboard');
    const { counts: tyreCounts, loading: tyreLoading } = useTyreCounts();
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    {tyreCounts.countsByLocation.map((loc) => (
                        ...
                        >
                            ...
                            <span className="text-xs text-gray-500 text-center truncate w-full">{loc.location}</span>
